...
import {
  Delete, EditPen, Search, CopyDocument, Edit, Plus
...
<template>
  <div class="container">
    <el-form :model="form" size="small" label-position="right" inline-message inline>
      ...
      <el-form-item>
        <el-button type="primary" size="small" @click="queryProfileList()" :icon="Search">{{ langData.btnSearch }}
        </el-button>
        <el-popconfirm :title="langData.profileFormBackupAllTips" confirm-button-type="warning" @confirm="backupAll">
          <template #reference>
            <el-button type="success" size="small" v-if="user.roleName=='ADMIN'" :icon="CopyDocument">
              {{ langData.profileFormBackupAll }}
            </el-button>
          </template>
        </el-popconfirm>
    ...
    <el-table :data="data.profileList" style="width: 100%" table-layout="fixed" :stripe="true"
      ...
      <el-table-column fixed="left" :label="langData.tableHeaderOp" width="120" header-align="center" align="center"
        ...
        <template #default="scope">
          <el-icon @click="showProfileEditDialog(scope)" color="#3F9EFF" style="cursor: pointer; margin-left: 10px"
                   :size="14" :title="langData.btnEdit">
            <Edit/>
          </el-icon>
          <el-popconfirm :title="langData.profileDeleteConfirmTitle" @confirm="deleteProfile(scope)"
            ...
            <template #reference>
              <el-icon color="red" style="cursor: pointer; margin-left: 10px" :size="14" :title="langData.btnDelete">
                <Delete/>
              </el-icon>
            </template>
          </el-popconfirm>
          <el-popconfirm :title="langData.profileRecoveryConfirmTitle" @confirm="backup(scope)"
                         ...
                         confirm-button-type="danger">
            <template #reference>
              <el-icon color="#3F9EFF" style="cursor: pointer; margin-left: 10px" :size="14"
                       :title="langData.profileTableOpBackup">
                <CopyDocument/>
              </el-icon>
            </template>
          </el-popconfirm>
      ...
      <el-table-column prop="profileName" :label="langData.tableHeaderName" :show-overflow-tooltip="true"
                       header-align="center"
                       align="center"/>
      <el-table-column prop="profileDesc" :label="langData.tableHeaderDesc" :show-overflow-tooltip="true"
                       header-align="center"
                       align="center"/>
      <el-table-column prop="username" :label="langData.profileTableHeaderCreator" :show-overflow-tooltip="true"
                       header-align="center"
                       align="center"/>
    ...
    <div class="addBtn">
      <el-button :icon="Plus" size="small" round @click="showProfileAddDialog"
                 v-if="user.roleName=='ADMIN'">{{ langData.btnAdd }}
      </el-button>
    </div>
