...
public class APIServiceImpl implements APIService {
    ...
    public Object getConfigList(APIModel model) {
        ...
        Map<String, ConfigEntity> profileConfigMap = profileConfigs.stream()
                .collect(Collectors.toMap(ConfigEntity::getConfigKey, e -> e, (k1, k2) -> k2));
        ...
        if (MapUtil.isNotEmpty(profileConfigMap)) {
            ...
            pairs = defaultConfigs.stream().map(e -> {
                ...
                if (profileConfig != null) {
                    pair = new TypePair(profileConfig.getConfigKey(),
                            profileConfig.getConfigValue(),
                            profileConfig.getDataType());
                } else {
                    pair = new TypePair(e.getConfigKey(),
                            e.getConfigValue(),
                            e.getDataType());
                }
                return pair;
            ...
            if(MapUtil.isNotEmpty(profileConfigMap)){
                for (ConfigEntity conf : profileConfigMap.values()) {
                    pairs.add(new TypePair(conf.getConfigKey(),
                            conf.getConfigValue(),
                            conf.getDataType()));
                }
            }
        } else {
            pairs = profileConfigs.stream()
                    .map(e -> new TypePair(e.getConfigKey(), e.getConfigValue(), e.getDataType()))
                    .collect(Collectors.toList());
        }
